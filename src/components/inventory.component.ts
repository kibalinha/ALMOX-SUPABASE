...
export class InventoryComponent implements OnDestroy {
  ...
  db: WritableSignal<AlmoxarifadoDB>;
  ...
  async saveBatchItems() {
    ...
    for (const itemGroup of this.batchItemsArray.controls) {
      ...
      const cleanedCnpj = itemValue.supplierCnpj?.replace(/[^\d]/g, '');
      const supplier = cleanedCnpj ? suppliers.find(s => s.cnpj?.replace(/[^\d]/g, '') === cleanedCnpj) : null;
