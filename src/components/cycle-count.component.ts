                    ...
                    <div class="bg-white dark:bg-primary p-6 rounded-lg shadow-md">
                        ...
                        <form [formGroup]="countForm">
                            ...
                            <div class="flex justify-end gap-4 mt-6">
                                ...
                                <!-- FIX: Fixed broken class attribute and removed extraneous pasted file content -->
...
export class CycleCountComponent implements OnDestroy {
    ...
    suggestionReason = signal<string>('');
    ...
    async startNewCount() {
        ...
        try {
            ...
            if (result && result.itemsToCount.length > 0) {
                ...
                const itemsForCount = allItems.filter(i => itemIds.has(i.id));
                this.itemsToCount.set(itemsForCount);
                this.suggestionReason.set(result.reasoning);
                this.setupCountForm();
                this.step.set('counting');
            } else {
                this.toastService.addToast('Não foi possível obter sugestões da IA.', 'error');
            }
        } catch (e) {
             this.toastService.addToast('Falha ao comunicar com a IA para sugestões.', 'error');
        } finally {
    ...
    reviewCount() {
        ...
        }).filter(d => d.difference !== 0 || d.countedQuantity > 0 || d.systemQuantity > 0); // show all counted items
    ...
    async confirmAdjustments() {
        ...
        try {
          ...
          this.toastService.addToast('Contagem finalizada e ajustes aplicados!', 'success');
          this.reset();
        } catch(e) {
    ...
    cancelStocktake() {
        if (confirm('Tem certeza que deseja cancelar a contagem? Nenhuma alteração será salva.')) {
            this.reset();
        }
    }
    ...
    reset() {
        ...
        this.toastService.addToast('Contagem cancelada.', 'info');
    }
    ...
    handleScannedCode(code: string) {
        ...
        if (itemIndex > -1) {
            ...
            if (rowElements.length > itemIndex) {
              const rowElement = rowElements[itemIndex] as HTMLElement;
