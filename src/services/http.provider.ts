...
export class HttpProvider extends DataProvider {
  ...
  async getInitialData(): Promise<AlmoxarifadoDB> {
    try {
      ...
      const [items, redShelfItems, technicians, suppliers, movements, categories, auditLogs, purchaseOrders, pickingLists, kits, reservations, users] = await Promise.all([
        ...
        firstValueFrom(this.http.get<Kit[]>(`${API_BASE_URL}/kits`)),
        firstValueFrom(this.http.get<Reservation[]>(`${API_BASE_URL}/reservations`)),
        firstValueFrom(this.http.get<User[]>(`${API_BASE_URL}/users`)),
      ]);
  ...
  addMultipleItems(itemsToAdd: (Omit<Item, "id" | "createdAt">)[], isRedShelf: boolean): Promise<(Item | RedShelfItem)[]> {
      ...
      return firstValueFrom(this.http.post<(Item|RedShelfItem)[]>(`${API_BASE_URL}/${endpoint}`, itemsToAdd));
  }
